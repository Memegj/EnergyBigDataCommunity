...
import upc.backend.controller.literature.Param.LiteratureAddParam;
...
public class LiteratureUpload {
    ...
    @RequestMapping(value = "/literature/files", method = RequestMethod.POST)
    public Result upload(HttpServletRequest httpServletRequest, @RequestParam("file") MultipartFile file) throws URISyntaxException, FileNotFoundException {
        ...
        File destFile = new File(Constants.LITERATURE_UPLOAD_DIC + newFileName); // 创建目标文件
        try {
            ...
            multipartData.add("/upload/literature/" + newFileName);
     ...
     * 图片上传
     */
    @RequestMapping(value = "/literature/images", method = RequestMethod.POST)
    ...
    @RequestMapping(value = "/literature/file_save", method = RequestMethod.POST)
    //@ApiOperation(value = "新增商品信息", notes = "新增商品信息")
    public Result save(@RequestBody LiteratureAddParam literatureAddParam, @RequestHeader("token") String str_token) {
        if (null != str_token && !"".equals(str_token) && str_token.length() == Constants.TOKEN_LENGTH) {
            ...
            if (userToken == null) {
                ...
                BeanUtil.copyProperties(literatureAddParam, literature);
                literature.setUserId(user.getUserId());
                literature.setUploadTime(new Date());
                Boolean literatureUpload_flag = literatureService.add_file(literature);
                if (literatureUpload_flag) {
    ...
    @RequestMapping(value = "/literature/teamlist", method = RequestMethod.GET)
