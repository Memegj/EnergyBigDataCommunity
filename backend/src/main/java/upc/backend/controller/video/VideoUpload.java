...
public class VideoUpload {
    ...
    @RequestMapping(value = "/video/files", method = RequestMethod.POST)
    public Result upload(HttpServletRequest httpServletRequest, @RequestParam("file") MultipartFile file) throws URISyntaxException, FileNotFoundException {
        ...
        File destFile = new File(Constants.VIDEO_UPLOAD_DIC + newFileName); // 创建目标文件
        try {
            ...
            multipartData.add("/upload/video/" + newFileName);
     ...
     * 图片上传
     */
    @RequestMapping(value = "/video/images", method = RequestMethod.POST)
    ...
    @RequestMapping(value = "/video/file_save", method = RequestMethod.POST)
    //@ApiOperation(value = "新增商品信息", notes = "新增商品信息")
    public Result save(@RequestBody VideoAddParam videoAddParam, @RequestHeader("token") String str_token) {
        if (null != str_token && !"".equals(str_token) && str_token.length() == Constants.TOKEN_LENGTH) {
            ...
            if (userToken == null) {
                ...
                BeanUtil.copyProperties(videoAddParam, video);
                video.setUserId(user.getUserId());
                video.setUploadTime(new Date());
                Boolean videoUpload_flag = videoService.add_file(video);
                if (videoUpload_flag) {
    ...
    @RequestMapping(value = "/video/teamlist", method = RequestMethod.GET)
